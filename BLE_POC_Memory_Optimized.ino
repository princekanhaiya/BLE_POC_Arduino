...
void setup() {
  //All CAN messages
  SEARCHCAR       = {SEARCHCAR_ID,    { 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00}, 03, 10 };
  ...
  TRUNKOPEN       = {TRUNKOPEN_ID,    { 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, 03, 10 };
  ...
  CAN0.init_Mask(1, 0, 0x037F0000);              // Init second mask...
  CAN0.init_Filt(3, 0, 0x01240000);
  CAN0.init_Filt(4, 0, 0x01270000);
  CAN0.init_Filt(5, 0, 0x02780000);
  CAN0.init_Filt(6, 0, 0x02140000);
  CAN0.init_Filt(7, 0, 0x03530000);
...
void loop() {
  ...
  if (nfc_auth) {
    ...
    if (currentMillis[0] - previousMillis[0] >= autoAuthFail) {
      nfc_auth = false;
      lcdWrite(F(" AUTH FAILED"));
      lcd.setCursor(0, 1);
      lcd.print(F(" PLACE AGAIN TAG"));
  ...
  if(ssbCurrentVoltage<2.80 && ssbCurrentVoltage> 2){
    countSSB++;
    Serial.println(countSSB);
  }
...
//  if ( (buttonPushCounter > 2 || countSSB>135)  && nfc_auth && gearStatus && !ignitionStatus && !crankStatus) {
//    digitalWrite(redLED, HIGH);
//    lcdWrite("  IGNITION ON   ");
//    digitalWrite(ignRel, HIGH);
//    ignitionStatus = true;
...
void bt_comm() {
  if (bleSerial.available() > 0) {
    ...
    if (btData == F("search")) {
      canWrite(SEARCHCAR, F("search"));
      lcdWrite(F(" SEARCHING CAR  "));
    }
    if (btData == F("windowup")) {
      canWrite(WINDOWUP, F("windowup"));
      lcdWrite(F(" POWER WINDOW UP"));
    }
    if (btData == F("windowdown")) {
      canWrite(WINDOWDOWN, "windowdown");
      lcdWrite(F("POWER WINDOW DWN"));
    }
    if (btData == F("wiperopen")) {
      canWrite(WIPEROPEN, "wiperopen");
      lcdWrite(F(" WIPER RUNNING  "));
    }
    if (btData == F("trunkopen")) {
      canWrite(TRUNKOPEN, "trunkopen");
      lcdWrite(F(" TRUNK OPENING  "));
    }
    if (btData == F("sunroofopen")) {
      canWrite(SUNROOFOPEN, "sunroofopen");
      lcdWrite(F(" SUNROOF OPENED "));
    }
    if (btData == F("sunroofclose")) {
      canWrite(SUNROOFCLOSE, F("sunroofclose"));
      lcdWrite(F(" SUNROOF CLOSED "));
    }
    if (btData == F("gateopen")) {
      canWrite(GATEUNLOCK, "gateopen");
      lcdWrite(F("VEHICLE UNLOCKED"));
    }
    if (btData == F("gateclose")) {
      canWrite(GATELOCK, "gateclose");
      lcdWrite(F(" VEHICLE LOCKED "));
    }
    if (btData == F("acopen")) {
    ...
    if (btData == F("ignon")) {
      ...
      lcdWrite(F("  IGNITION ON   "));
      digitalWrite(ignRel, HIGH);
    }
    if (btData == F("ignoff")) {
      //      canWrite(IGNOFF, "ignoff");
      digitalWrite(ignRel, LOW);
      lcdWrite(F("  IGNITION OFF  "));
    }
    if (btData == F("crankon")) {
    ...
    if (btData == F("crankoff")) {
      //      canWrite(CRANKOFF, "crankoff");
      digitalWrite(crankRel, LOW);
      lcdWrite(F("  CRANK OFF  "));
    }
    if (btData == F("reset")) {
      Serial.println(F("Wait ECU is resetting"));
      delay(100);
...
void canRead() {
  ...
  Serial.print(F("CAN ID: 0x")); Serial.print(rxId, HEX);
    ...
    case NFC_ENGINE_MODULE_ID://CAN ID = 0x6EB for Engine Ignition.
      if (rxBuf[0] == 0x03) {
        ...
        if (currentMillis[3] - previousMillis[3] <= 500) {
          Serial.println(F("returning from authentication"));
          return;
        }
    ...
    case NFC_CO_DRIVER_DOOR_MODULE_ID://CAN ID = 0x6EA for co driver side door access.
      if (rxBuf[0] == 0x03) {
        ...
        if (currentMillis[2] - previousMillis[2] <= nfcCardPlacmentTime) {
          Serial.println(F("returning co door handling"));
          return;
        }
        previousMillis[2] = currentMillis[2];
        NFC_driver_door_status = !NFC_driver_door_status;
        if (NFC_driver_door_status) {
          Serial.println(F("Driver door open"));
          canWrite(GATEUNLOCK, "gateopen");
          lcdWrite("VEHICLE UNLOCKED");
        }
        if (!NFC_driver_door_status) {
          Serial.println(F("Driver door close"));
          canWrite(GATELOCK, "gateclose");
          lcdWrite(" VEHICLE LOCKED ");
        }
      }
      break;
    case NFC_DRIVER_DOOR_MODULE_ID://CAN ID = 0X6E9 for driver side door access.
      if (rxBuf[0] == 0x03) {
        ...
        if (currentMillis[1] - previousMillis[1] <= nfcCardPlacmentTime) {
          Serial.println(F("returning from door handlling"));
          return;
        }
